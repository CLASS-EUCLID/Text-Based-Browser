...
int dll_get_size(doubly_linked_list_t *list) { return list->size; }

// the memory of the node is not freed in this function
...
void dll_print_int_list(doubly_linked_list_t *list, FILE *out) {
    if (!list->head) {
        return;
    }
...
void dll_print_string_list(doubly_linked_list_t *list, FILE *out) {
    if (!list->head) {
        return;
    }
    dll_node_t *p = list->head;
...
unsigned int st_get_size(stack_t *st) { return dll_get_size(st->list); }
...
// allocates mem for the browser, the sentinel and the intial tab
// the sentinel and the initial cannot be closed
...
C - CURRENT
F - FORWARD
this is how the stacks look with 1 being the oldest and 7 being the youngest tab opened
these next two functions basically move left/right if we consider the line of tabs to be 1 2 3 4 5 6 7
B C F
3 4 5
2   6  
1   7
...
// PRINT (all id's STARTING WITH THE CURRENT ONE)
...
// PRINT_HISTORY <ID> (FROM YOUNGEST TO OLDEST (LINE 337 for some details))
...
int main(void) {
    ...
    for (int i = 0; i < queries; i++) {
        ...
        if (!strcmp(query, "NEW_TAB")) {
            ...
            char tab_num[20];
            strcpy(tab_num, query + 5); // offset to take only the numbers
            ...
            char tab_num[20];
            strcpy(tab_num, query + 13); // offset to take only the numbers
